...
public class AchievementsController : Controller
{
    ...
    public async Task<IActionResult> CheckAchievements()
    {
        ...
        // Get user statistics
        var problemsCount = await _context.HistoryItems.CountAsync(h => h.UserId == userId);
        var correctAnswers = await _context.HistoryItems.CountAsync(h => h.UserId == userId && h.IsCorrect);
        var studySessions = await _context.StudySessions.CountAsync(s => s.UserId == userId);
        ...
        if (problemsCount >= 1)
        {
            var achievement = await GetOrCreateAchievement(userId, "first_problem", "First Problem", "Solved your first problem!", "common");
        ...
        if (problemsCount >= 10)
        {
            var achievement = await GetOrCreateAchievement(userId, "problem_solver", "Problem Solver", "Solved 10 problems!", "rare");
            if (!achievement.IsUnlocked)
            {
                achievement.IsUnlocked = true;
                achievement.UnlockedAt = DateTime.UtcNow;
        ...
        {
            var achievement = await GetOrCreateAchievement(userId, "math_master", "Math Master", "Solved 100 problems!", "epic");
            if (!achievement.IsUnlocked)
            {
                achievement.IsUnlocked = true;
                achievement.UnlockedAt = DateTime.UtcNow;
        ...
        {
            var achievement = await GetOrCreateAchievement(userId, "perfect_week", "Perfect Week", "Studied for 7 days straight!", "legendary");
            if (!achievement.IsUnlocked)
            {
                achievement.IsUnlocked = true;
                achievement.UnlockedAt = DateTime.UtcNow;
